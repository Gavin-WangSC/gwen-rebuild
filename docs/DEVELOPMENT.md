...
| Context recovery for Codex | Complete on `codex/context-recovery` | Tracked agent map, this document, project MCP and safety setup                |
| Runnable product slice     | Not started                          | Runner, operational CLI, provider adapter/config, and viewer remain           |
...
- `src/lib/pipeline/`: validated essay input, fixed prompts, executable 16-step dependency graph,
  in-essay wave scheduling, injected model boundary, retries, score extraction, and typed results.
...
- 102 offline tests at the time of this recovery pass. The exact current count comes from
  `bun run check`, not from this sentence.

The pipeline remains persistence-free. It emits step results through an injected callback; the
future runner owns SQLite persistence, resume behavior, cross-essay scheduling, and process
lifecycle. The CLI owns arguments, environment lookup, stdout/stderr, and exit codes. Routes read
the database and never execute grading.
...
1. Define provider-neutral runner and provider configuration boundaries without choosing through
...
4. Persist step results and final result, then resume safely after an injected failure.
